...
export class secondscreencitation extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
     ...
     var person = realm.objects('Add_Person').filtered('reportids==3').filtered('fieldids ==  $0',this.props.navigation.state.params.ID).sorted('person_id');
    var vehicle = realm.objects('Add_Vehicle').filtered('reportids==3').filtered('fieldids ==  $0',this.props.navigation.state.params.ID).sorted('vehicle_id');
    var charge = realm.objects('Add_Charge_Citation').filtered('fieldids ==  $0',this.props.navigation.state.params.ID).sorted('charge_id');

    var ds = new ListView.DataSource({ rowHasChanged: (r1, r2) => r1 !== r2 });
    this.state = {
      persondatasrc : ds.cloneWithRows(person),
      vehicledatasrc : ds.cloneWithRows(vehicle),
      citchargedatasrc : ds.cloneWithRows(charge),
    }
    ...
    realm.addListener('change', () => {
      this.setState({
        ...
        citchargedatasrc : this.state.citchargedatasrc.cloneWithRows(charge),
      })
    });
  ...
  Cancel_Employee = () => {
    this.setState({
      ...
      CI_ID: this.props.navigation.state.params.ID,
      ...
      Employee_CITCHARGE: this.props.navigation.state.params.ARRAYCHARGE,
  ...
  GoToaddpersonActivity(reportid,id,personid, firstname, lastname, middlename, country, state, city, postalcode, streetaddress, apartmentnumber, age, license, ssn, gender, eyes, hair, height, weight, race, ethnicity, citizenship, homephone, workphone, cellphone, otherphone, personimage) {
  ...
  GoToaddvehicleActivity(reportid,id,VEHICLEIDS,plate, plateyear, platetype, platestate, vehyear, color, color2, vin, bodystyle, bodytype, notes, image) {
  ...
  GoToaddchargeActivity(bookid,id,chargeviolation,chargeordinance,courtdate,compliancedate,fee,comments) {

    this.setState({ savepress: false });
    this.props.navigation.navigate('addchargecitation', {
      FIELDID:bookid,
      ID: id,
      CHARGEVIOLATION:chargeviolation,
      CHARGEORDINANCE:chargeordinance,
      COURTDATE:courtdate,
  ...
  GoToLocationActivity(reportid,id, location_country, location_state, location_city, location_postalcode, location_streetaddress, location_apartmentnumber, ) {
    this.setState({ savepress: false });
  ...
  GoToNarrationActivity(reportid,id,narrationtext){
        this.setState({ savepress: false })
        ...
        { text: 'NO', onPress: () => console.log('Cancel Pressed'), style: 'cancel' },
        {
          ...
          text: 'YES', onPress: () => {
             ...
             realm.write(() => {
              ...
              var ID = this.state.CI_ID - 1;
              this.setState({ CI_ID: this.state.CI_ID - 1 })
              realm.delete(realm.objects('Citation_Info')[ID]);
              realm.delete(realm.objects('Add_Person').filtered('reportids == 3').filtered('fieldids ==  $0',ID + 1));
              realm.delete(realm.objects('Add_Vehicle').filtered('reportids == 3').filtered('fieldids ==  $0',ID + 1));
              realm.delete(realm.objects('Add_Charge_Citation').filtered('fieldids ==  $0',ID + 1));


              if(realm.objects('Citation_Info').length != 0)
                {
                for (i = 1; i <= realm.objects('Citation_Info').length; i++) {
                realm.objects('Citation_Info')[i - 1].CitationID = i;
                }}
            ...
            if(realm.objects('Add_Person').length != 0)
                {
                   for (i = ID+2; i <= realm.objects('Citation_Info').length+1; i++) {
                realm.objects('Add_Person').filtered('reportids == 3').filtered('fieldids == $0',i)[0].fieldids = i-1;}
              for(i=1;i<=realm.objects('Add_Person').length;i++){
               realm.objects('Add_Person')[i-1].person_id = i
                ...
                { for (i = ID+2; i <= realm.objects('Citation_Info').length+1; i++) {
                realm.objects('Add_Vehicle').filtered('reportids == 3').filtered('fieldids == $0',i)[0].fieldids = i-1;}
              for(i=1;i<=realm.objects('Add_Vehicle').length;i++){
               realm.objects('Add_Vehicle')[i-1].vehicle_id = i
               }}


               if(realm.objects('Add_Charge_Citation').length != 0)
                {
                  for (i = ID+2; i <= realm.objects('Citation_Info').length+1; i++) {

              for(j=0;j<=realm.objects('Add_Charge_Citation').filtered('fieldids == $0',i).length-1;j++){
                realm.objects('Add_Charge_Citation').filtered('fieldids == $0',i)[j].fieldids = i-1;
              }
              }
  ...
  namelistview = () => {
    ...
    var ID = this.state.CI_ID ;
    var obj = realm.objects('Citation_Info');
    if (realm.objects('Add_Person').filtered('reportids==3').filtered('fieldids ==  $0',this.props.navigation.state.params.ID).length == 0) {
      return (
        <View><View style={{ backgroundColor: '#D3D3D3', height: 35 }}><Text style={{ paddingHorizontal: 30, fontSize: 20 }}>PERSON</Text></View>
          <View style={{ flex: 1, }} >
            <TouchableOpacity onPress={this.GoToaddpersonActivity.bind(this,3, ID,realm.objects('Add_Person').length, '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', 'https://lh3.googleusercontent.com/0Kgou0TtfKxvX2wPezXfbZwxmR6qAWEdemPNB57dQrjyRHC42cIPzEbESf2PDcJCc7WGO79dwsnCq-_6XaHnhmKyZJ8=s1024')} >
    ...
    else if(realm.objects('Add_Person').filtered('reportids==3').filtered('fieldids ==  $0',this.props.navigation.state.params.ID).length == 1){
      return (
  ...
  chargelistview = () => {
    ...
    var ID = this.state.CI_ID ;
    var obj = realm.objects('Citation_Info');
    if (realm.objects('Add_Charge_Citation').filtered('fieldids ==  $0',this.props.navigation.state.params.ID).length == 0) {
      return (
        ...
        <View><View style={{ backgroundColor: '#D3D3D3', height: 50 }}><Text style={{ paddingHorizontal: 30, fontSize: 20 }}>CHARGE</Text></View>
          <View style={{ flex: 1 }} >
            <ListView
              ...
              dataSource={this.state.citchargedatasrc}
              enableEmptySections={true}
              RefreshList={this.onRefreshList}
              renderSeparator={this.ListViewItemSeparator}
              ...
              renderRow={(rowData) => <View style={{ flex: 1 }} >
                ...
                <TouchableOpacity onPress={this.GoToaddchargeActivity.bind(this, ID,rowData.charge_id, rowData.charge_violation, rowData.charge_ordinance, rowData.charge_Court_Date, rowData.charge_Compliance_Date, rowData.charge_Fee_Amount,rowData.charge_Comments)} >
                  <View style={styles.ganesh}>
                    ...
                    <View>
                      ...
                      <Text style={styles.textViewContainer} >{rowData.charge_ordinance}</Text>
            ...
            <TouchableOpacity onPress={this.GoToaddchargeActivity.bind(this, ID,realm.objects('Add_Charge_Citation').length, '', '', '', '', '','')} >
   ...
   vehiclelistview = () => {
    var ID = this.state.CI_ID ;
    var obj = realm.objects('Citation_Info');
    if (realm.objects('Add_Vehicle').filtered('reportids==3').filtered('fieldids ==  $0',this.props.navigation.state.params.ID).length == 0) {
      return (
        <View><View style={{ backgroundColor: '#D3D3D3', height: 35 }}><Text style={{ paddingHorizontal: 30, fontSize: 20 }}>VEHICLE</Text></View>
          <View style={{ flex: 1, }} >
            <TouchableOpacity onPress={this.GoToaddvehicleActivity.bind(this, 3,ID,realm.objects('Add_Vehicle').length, '', '', '', '', '', '', '', '', '', '', '', 'https://lh3.googleusercontent.com/VD_6uiGvWgA1O4nVUukE2caj2YdhsizINJaXuhgcS4HuJazKADGcztO8BqWxJOiYjLH7FQhsBPEeF25ifnFd_pBu=s1024')} >
    ...
    else if (realm.objects('Add_Vehicle').filtered('reportids==3').filtered('fieldids ==  $0',this.props.navigation.state.params.ID).length == 1){
      return (
          ...
          <View style={{ flex: 1 }} >
            <ListView
              ...
              renderRow={(rowData) => <View style={{ flex: 1 }} >
                ...
                <TouchableOpacity onPress={this.GoToaddvehicleActivity.bind(this,3, ID,rowData.vehicle_id, rowData.vehicle_plate, rowData.vehicle_plateyear, rowData.vehicle_platetype, rowData.vehicle_patestate, rowData.vehicle_year, rowData.vehicle_Color, rowData.vehicle_Color2, rowData.vehicle_VIN, rowData.vehicle_BodyStyle, rowData.vehicle_Bodytype, rowData.vehicle_Notes, rowData.vehicle_image)} >
                  <View style={styles.ganesh}>
  ...
  locationlistview = () => {
    ...
    var obj = realm.objects('Citation_Info');
    if(ID >= 0){
    if (obj[ID].location_apartmentnumber == '' && obj[ID].location_streetaddress == '' && obj[ID].location_postalcode == '' && obj[ID].location_city == '' && obj[ID].location_state == '' && obj[ID].location_country == '') {
      return (
        <View><View style={{ backgroundColor: '#D3D3D3', height: 35 }}><Text style={{ paddingHorizontal: 30, fontSize: 20 }}>LOCATION</Text></View>
          <View style={{ flex: 1, }} >
            <TouchableOpacity onPress={this.GoToLocationActivity.bind(this,3, obj[ID].CitationID, obj[ID].location_country, obj[ID].location_state, obj[ID].location_city, obj[ID].location_postalcode, obj[ID].location_streetaddress, obj[ID].location_apartmentnumber)} >
    ...
    if (obj[ID].location_narrative == '') {
      return (
        <View><View style={{ backgroundColor: '#D3D3D3', height: 35 }}><Text style={{ paddingHorizontal: 30, fontSize: 20 }}>NARRATIVE</Text></View>
          <View style={{ flex: 1, }} >
            <TouchableOpacity onPress={this.GoToNarrationActivity.bind(this,3, obj[ID].CitationID, obj[ID].location_narrative)} >
    ...
    if (obj[ID].location_image == 'https://lh3.googleusercontent.com/7byfpPoWE_v1Lzm3nTCvlD_hELIOF7b8GJ7wLphbSm98hJtyO6KBpx4lVl9OR8POsc964W2QsVAUayTiwgJckyhWGA=s1024') {
      return (
        <View><View style={{ backgroundColor: '#D3D3D3', height: 35 }}><Text style={{ paddingHorizontal: 30, fontSize: 20 }}>ATTACHMENT</Text></View>
          <View style={{ flex: 1, }} >
            <TouchableOpacity onPress={this.GoToAttachmentActivity.bind(this,3, obj[ID].CitationID, obj[ID].location_image)} >
      ...
      <View>
        <View style={styles.header}>
          ...
          <View style={styles.whitespace}>
            <Text style={{ fontSize: 20, fontWeight: 'bold' }}>Citation</Text></View>
...
export default secondscreencitation;